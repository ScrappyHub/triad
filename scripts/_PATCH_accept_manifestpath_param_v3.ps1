...
$BackupDir = Join-Path $ScriptsDir ("_backup_manifestpath_param_v3_" + $ts)
...
foreach($p in $targets){
  ...
  if ($has) { Parse-GateFile $p; Write-Host ("OK: param block already declares " + '$ManifestPath' + ": " + $p) -ForegroundColor Green; continue }

  # Insert after SnapshotDir param if present, else insert right after param(
  $idx=-1
  ...
  if ($idx -ne $start) {
    ...
    $insIndent = ([regex]::Match($lines[$idx], '^(\s*)')).Groups[1].Value
    if ([string]::IsNullOrWhiteSpace($insIndent)) { $insIndent = "  " }
  } else {
    $insIndent = "  "
  }
  ...
  for($i=0;$i -lt $lines.Count;$i++){
    ...
    if ($i -eq $idx) { [void]$out.Add($insIndent + '[string]$ManifestPath,') }
  }
  ...
  Write-Host ("PATCHED: inserted [string]" + '$ManifestPath' + " in param block: " + $p) -ForegroundColor Cyan
}

...
$BackupDir = Join-Path $ScriptsDir ("_backup_gate1_fix_v9_" + $ts)
...
# Fix: replace ONLY the exact bad line: "`$InputDir = $null" -> "$InputDir = $null"
$badPat = '(?m)^[\t ]*`\$InputDir[\t ]*=[\t ]*\$null[\t ]*$'
$good   = '$InputDir = $null'
$beforeCount = ([regex]::Matches($txt,$badPat)).Count
if ($beforeCount -lt 1) { Die "GATE1_V9_NO_MATCH: did not find exact bad line (`$InputDir = $null) to fix" }
$txt2 = [regex]::Replace($txt,$badPat,$good)

# Safety: after replacement, there must be ZERO occurrences of backtick-escaped `$InputDir
$remPat = '`\$InputDir'
$remCount = ([regex]::Matches($txt2,$remPat)).Count
if ($remCount -ne 0) { Die ("GATE1_V9_REMAINING_BACKTICK_INPUTDIR: " + $remCount) }

Write-Utf8NoBomLf $Target $txt2
Parse-GateFile $Target
Write-Host "OK: Gate1 v9 removed stray backtick before $InputDir assignment + parse-gated patch" -ForegroundColor Green
Write-Host ("target:    {0}" -f $Target) -ForegroundColor Cyan
Write-Host ("backupdir: {0}" -f $BackupDir) -ForegroundColor DarkGray
Write-Host ("fixed_lines_matched: {0}" -f $beforeCount) -ForegroundColor Cyan

...
# Insert: literal $InputDir = $null right after Set-StrictMode (only if missing)
if($txt -notmatch "(?m)^[`t ]*\\$InputDir[`t ]*="){
  $lines = @($txt -split "`n",-1)
  ...
  for($i=0;$i -lt $lines.Count;$i++){
    $ln = $lines[$i]
    [void]$out.Add($ln)
    if(-not $inserted -and $ln -match "^[`t ]*Set-StrictMode[`t ]+-Version[`t ]+Latest[`t ]*$"){

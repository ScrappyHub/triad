...
$BackupBytes = [System.IO.File]::ReadAllBytes($FirstBlockPath)
$QuarantinePath = $FirstBlockPath + ".missing_test_hold"
...
try {
  ...
  Move-Item -LiteralPath $FirstBlockPath -Destination $QuarantinePath -Force
  ...
  if(($out -notmatch "MISSING_BLOCK") -and ($out -notmatch "MISSING_TARGET") -and ($out -notmatch "MISSING_FILE")){
    Die ("EXPECTED_FAILURE_TOKEN_NOT_FOUND: " + $out)
  }

  Write-Host ("NEG_PLAN: " + $PlanPath) -ForegroundColor DarkGray
  Write-Host ("NEG_MISSING_BLOCK: " + $FirstBlockPath) -ForegroundColor DarkGray
  Write-Host "TRIAD_NEGATIVE_VECTOR_MISSING_BLOCK_V1_SELFTEST_OK" -ForegroundColor Green
}
finally {
  if((Test-Path -LiteralPath $QuarantinePath -PathType Leaf) -and -not (Test-Path -LiteralPath $FirstBlockPath -PathType Leaf)){
    Move-Item -LiteralPath $QuarantinePath -Destination $FirstBlockPath -Force
  } elseif(-not (Test-Path -LiteralPath $FirstBlockPath -PathType Leaf)){
    [System.IO.File]::WriteAllBytes($FirstBlockPath,$BackupBytes)
    if(Test-Path -LiteralPath $QuarantinePath -PathType Leaf){ Remove-Item -LiteralPath $QuarantinePath -Force }

...
$BackupDir = Join-Path $ScriptsDir ("_backup_patch_inputdir_v7_" + $ts)
...
# Disable ONLY the known-bad invocations: & '$InputDir'  OR  . '$InputDir'
$out = New-Object System.Collections.Generic.List[string]
$changed = 0
for($i=0;$i -lt $lines.Count;$i++){
  $ln = $lines[$i]
  if($ln -match $patInvoke){
    [void]$out.Add("# FIX_V7: disabled literal invocation of `$InputDir: " + $ln)
    $changed++
  } else {
    [void]$out.Add($ln)
  }
...
Parse-Gate $Target
Write-Host "OK: inputdir v7 report written + invocation lines disabled + parse-gated" -ForegroundColor Green

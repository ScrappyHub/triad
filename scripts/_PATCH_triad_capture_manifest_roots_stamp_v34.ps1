...
$BackupDir = Join-Path $ScriptsDir ("_backup_triad_capture_v34_" + $ts)
...
for($i=0;$i -lt $lines.Count;$i++){
  if ($lines[$i] -match '(?im)^\s*#\s*PATCH_MAN_ROOTS_STAMP_V34\s*$'){
    Parse-GateFile $Target
    Write-Host ("OK: v34 already present: " + $Target) -ForegroundColor Green
    return
  }
}

# Robust anchor:
# 1) find first mention of "manifest.json"
$manRef = -1
for($i=0;$i -lt $lines.Count;$i++){
  if ($lines[$i] -match '(?im)manifest\.json'){ $manRef = $i; break }
}
if ($manRef -lt 0) { Die "NO_MANIFEST_JSON_MENTION_FOUND_V34" }

# 2) prefer a nearby write-ish line (same line or within next 8 lines)
$ix = -1
$max = [Math]::Min($lines.Count-1, $manRef + 8)
for($j=$manRef; $j -le $max; $j++){
  if ($lines[$j] -match '(?im)\b(Write-|WriteUtf8|Write\-Utf8|WriteAllText|Set\-Content|Out\-File)\b'){
    $ix = $j; break
  }
}

# 3) If none, insert at the manifest mention itself
if ($ix -lt 0) { $ix = $manRef }

$indent = ([regex]::Match($lines[$ix], '^(\s*)')).Groups[1].Value

$ins = New-Object System.Collections.Generic.List[string]
[void]$ins.Add($indent + '# PATCH_MAN_ROOTS_STAMP_V34')
...
Parse-GateFile $Target
Write-Host ("FINAL_OK: patched " + $Target + " (v34 stamped man.roots.{block_root,semantic_root}; insertLine=" + $ix + "; manRef=" + $manRef + ")") -ForegroundColor Green

...
$WorkRoot     = Join-Path $RepoRoot "scripts\_work\triad_transform_negative_transform_id_mismatch_v1"
...
$m.transform_id = ("0" * 64)
Write-Utf8NoBomLf $ManifestPath (($m | ConvertTo-Json -Depth 20 -Compress))
...
if($r2.Output -notmatch "TRANSFORM_ID_MISMATCH"){ Die ("EXPECTED_FAILURE_TOKEN_NOT_FOUND: " + $r2.Output) }

Write-Host ("NEG_MANIFEST: " + $ManifestPath) -ForegroundColor DarkGray
Write-Host "TRIAD_TRANSFORM_NEGATIVE_TRANSFORM_ID_MISMATCH_V1_SELFTEST_OK" -ForegroundColor Green

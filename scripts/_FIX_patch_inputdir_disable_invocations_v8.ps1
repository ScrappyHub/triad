...
$BackupDir = Join-Path $ScriptsDir ("_backup_patch_inputdir_v8_" + $ts)
...
# Report: all lines containing literal "$InputDir" (no expansion)
$ReportPath = Join-Path $BackupDir "inputdir_hits.txt"
$hits = New-Object System.Collections.Generic.List[string]
for($i=0;$i -lt $lines.Count;$i++){
  $ln = $lines[$i]
  if($ln -like "*`$InputDir*"){
    [void]$hits.Add(("{0:D5}: {1}" -f ($i+1), $ln))
  }
}
Write-Utf8NoBomLf $ReportPath (($hits.ToArray()) -join "`n")

# Disable ANY invocation of $InputDir:
# - & $InputDir
# - . $InputDir
# - & "$InputDir" / & '$InputDir'
# - . "$InputDir" / . '$InputDir'
# Also disable any line that begins with $InputDir but is not an assignment.
$out = New-Object System.Collections.Generic.List[string]
$changed = 0

$patInvokeAny = '^[\t ]*[&.][\t ]*(["'']?)\$InputDir\1([\t ]+|$)'
...
for($i=0;$i -lt $lines.Count;$i++){
  ...
  if(($ln -match $patInvokeAny) -or (($ln -match $patCmdPos) -and ($ln -notmatch $patAssign))){
    [void]$out.Add("# FIX_V8: disabled InputDir execution line: " + $ln)
    $changed++
  } else {
    [void]$out.Add($ln)
  }
...
Write-Host "OK: v8 report written + InputDir execution disabled + parse-gated" -ForegroundColor Green
...
Write-Host ("hits_count:    {0}" -f $hits.Count) -ForegroundColor Cyan

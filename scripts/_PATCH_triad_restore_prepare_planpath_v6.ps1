...
$BackupDir = Join-Path $ScriptsDir ("_backup_triad_restore_prepare_v6_" + $ts)
...
# Locate our v5 injected block by markers
$begin = -1
$end = -1
for($i=0;$i -lt $lines.Count;$i++){
  if ($lines[$i] -match '(?im)^\s*#\s*---\s*patched\s*by\s*_PATCH_triad_restore_prepare_return_plan_v5\.ps1\s*---\s*$'){ $begin=$i; break }
}
if ($begin -lt 0) { Die "NO_V5_PATCH_BEGIN_MARKER_FOUND" }
for($i=$begin+1;$i -lt $lines.Count;$i++){
  if ($lines[$i] -match '(?im)^\s*#\s*---\s*end\s*patch\s*---\s*$'){ $end=$i; break }
}
if ($end -lt 0) { Die "NO_V5_PATCH_END_MARKER_FOUND" }

$indent = ([regex]::Match($lines[$begin], '^(\s*)')).Groups[1].Value

# Build v6 block:
# - keep user's $OutFile as final restore target
# - compute a separate plan json path adjacent to OutFile
# - call tree_prepare with -OutFile $PlanPath
# - return parsed plan object (blocks materialized)
$blk = New-Object System.Collections.Generic.List[string]
[void]$blk.Add($indent + '# --- patched by _PATCH_triad_restore_prepare_planpath_v6.ps1 ---')
[void]$blk.Add($indent + '$ts_plan = (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")')
[void]$blk.Add($indent + '$guid_plan = ([guid]::NewGuid().ToString("N"))')
...
[void]$blk.Add($indent + '$PlanPath = Join-Path $outParent ($outLeaf + ".triad_plan_tree_v1_" + $ts_plan + "_" + $guid_plan + ".json")')
[void]$blk.Add($indent + '$null = (& $TreePrep -RepoRoot $RepoRoot -SnapshotDir $SnapshotDir -OutFile $PlanPath -ManifestPath $ManifestPath)')
[void]$blk.Add($indent + 'if (-not (Test-Path -LiteralPath $PlanPath -PathType Leaf)) { throw ("MISSING_PLAN_OUTFILE: " + $PlanPath) }')
...
for($i=0;$i -lt $lines.Count;$i++){
  if ($i -eq $begin) {
    ...
    # skip old v5 block through end marker line
    $i = $end
    continue
  }
...
Write-Host ("FINAL_OK: patched " + $Target + " (replaced v5 block lines " + $begin + ".." + $end + ")") -ForegroundColor Green

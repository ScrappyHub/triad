...
$BackupDir = Join-Path $ScriptsDir ("_backup_triad_restore_prepare_v11_" + $ts)
...
# Find the 'return $planObj' line inside that block
$ret = -1
for($i=$begin+1;$i -lt $lines.Count;$i++){
  if ($lines[$i] -match '(?im)^\s*return\s+\$planObj\s*$'){ $ret = $i; break }
  if ($lines[$i] -match '(?im)^\s*#\s*---\s*end\s*patch\s*---\s*$'){ break }
}
if ($ret -lt 0) { Die "NO_RETURN_PLANOBJ_LINE_FOUND_IN_V7_BLOCK" }

$indent = ([regex]::Match($lines[$ret], '^(\s*)')).Groups[1].Value

# If a length normalization already exists, do nothing (idempotent)
$already = $false
for($i=$begin;$i -le $ret;$i++){
  if ($lines[$i] -match '(?i)planObj\)\s*\|\s*Add-Member.*\bname\s*"?length"?' -or
      $lines[$i] -match '(?i)\$planObj\.length\b'){
    $already = $true; break
  }
}
...
if ($already) {
  ...
  Write-Host ("OK: plan length normalization already present in v7 block: " + $Target) -ForegroundColor Green
  return
}

# Insert length normalization block immediately BEFORE return $planObj
$ins = New-Object System.Collections.Generic.List[string]
[void]$ins.Add($indent + '# length normalize: ensure planObj.length exists (StrictMode-safe)')
...
Parse-GateFile $Target
Write-Host ("FINAL_OK: patched " + $Target + " (inserted planObj.length normalization before return at line index " + $ret + ")") -ForegroundColor Green

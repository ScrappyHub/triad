...
$BackupDir = Join-Path $ScriptsDir ("_backup_patch_inputdir_v7b_" + $ts)
...
# Disable ONLY the known-bad literal invocations:
#   & '$InputDir'
#   . '$InputDir'
...
for($i=0;$i -lt $lines.Count;$i++){
  ...
  if($ln -match $patInvoke){
    [void]$out.Add("# FIX_V7B: disabled literal invocation of `$InputDir: " + $ln)
    $changed++
  } else {
    [void]$out.Add($ln)
  }
...
Write-Host "OK: v7B report written + invocation lines disabled + parse-gated" -ForegroundColor Green

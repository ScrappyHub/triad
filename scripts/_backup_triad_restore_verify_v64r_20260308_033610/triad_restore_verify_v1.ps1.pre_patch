...
")
          ...
          $b2a = @(@($b2))
          if($b2a.Count -gt 0){
            Write-Host ("TRACE_SIDECAR_BLOCKS_V61: " + $cand + " count=" + $b2a.Count) -ForegroundColor DarkGray
            return $b2
          }
        } catch { }
      }
  ...
  if(-not [string]::IsNullOrWhiteSpace($snapshotDir)){
    ...
    if(Test-Path -LiteralPath $blocksDir -PathType Container){
      ...
      if($files.Count -gt 0){
        ...
        Write-Host ("TRACE_BLOCKS_DIR_ENUM_V61: " + $blocksDir + " count=" + $out2.Count) -ForegroundColor DarkGray
        return @(@($out2.ToArray()))
      }
    }
  }
...
# PATCH_VERIFY_TRAP_V46C
trap {
  $e = $_
  $inv = $e.InvocationInfo
  $ln = 0; $col = 0; $line = ""
  if($inv -ne $null){
    try { $ln = [int]$inv.ScriptLineNumber } catch { $ln = 0 }
    try { $col = [int]$inv.OffsetInLine } catch { $col = 0 }
    try { $line = [string]$inv.Line } catch { $line = "" }
  }
  $t = ""
  try { $t = $e.Exception.GetType().FullName } catch { $t = "System.Exception" }
  $msg = ""
  try { $msg = [string]$e.Exception.Message } catch { $msg = [string]$e.ToString() }
  throw ("VERIFY_TRAP: " + $ln + ":" + $col + " " + $t + " " + $msg + "`nLINE: " + $line)
}
# /PATCH_VERIFY_TRAP_V46C
...
Write-Host ("TRACE_PAYLOAD_ENTRY_V63R: path=" + [string](PropOf $payloadEntry "path")) -ForegroundColor DarkGray
...
Write-Host "TRACE_BEFORE_GETBLOCKS_V57" -ForegroundColor DarkGray
$blocks = @(@(PropOf $payloadEntry "blocks"))
if(@(@($blocks)).Count -lt 1){
  $blocks = @(@((GetBlocksForVerify $man $plan)))
}
Write-Host "TRACE_AFTER_GETBLOCKS_V57" -ForegroundColor DarkGray
Write-Host ("TRACE_BLOCKS_COUNT_V57: " + $blocks.Count) -ForegroundColor DarkGray
Write-Host "TRACE_BEFORE_BLOCK_FOREACH_V57" -ForegroundColor DarkGray

...
$BackupDir = Join-Path $ScriptsDir ("_backup_triad_restore_verify_v61_" + $ts)
...
if($raw -match 'PATCH_BLOCKS_DIR_ENUM_V61'){
  Parse-GateFile_Parser $Target
  Write-Host ("OK: v61 already present: " + $Target) -ForegroundColor Green
  return
}
...
if(-not $fm.Success){ Die "GETBLOCKS_FUNCTION_NOT_FOUND_V61" }
...
Parse-GateFile_Parser $Target
Write-Host ("FINAL_OK: patched " + $Target + " (v61: blocks directory enumeration fallback)") -ForegroundColor Green

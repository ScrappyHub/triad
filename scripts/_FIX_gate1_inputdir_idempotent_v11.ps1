...
$BackupDir = Join-Path $ScriptsDir ("_backup_gate1_fix_v11_" + $ts)
...
if ($before -lt 1) { Die 'GATE1_V11_NO_MATCH: backtick-escaped assignment not found (file contains `\$InputDir but not the expected line form)' }
...
if ($rem -ne 0) { Die ("GATE1_V11_REMAINING_BACKTICK_INPUTDIR: " + $rem) }

Write-Utf8NoBomLf $Target $txt2
Parse-GateFile $Target
Write-Host "OK: Gate1 v11 removed backtick-escaped `$InputDir assignment + parse-gated patch" -ForegroundColor Green
Write-Host ("target:    {0}" -f $Target) -ForegroundColor Cyan
Write-Host ("backupdir: {0}" -f $BackupDir) -ForegroundColor DarkGray

...
$WorkRoot     = Join-Path $RepoRoot "scripts\_work\triad_transform_negative_input_sha_mismatch_v1"
...
Write-Utf8NoBomLf $InputPath "mutated input after manifest"
...
if($r2.Output -notmatch "INPUT_SHA_MISMATCH"){ Die ("EXPECTED_FAILURE_TOKEN_NOT_FOUND: " + $r2.Output) }

Write-Host ("NEG_MANIFEST: " + $ManifestPath) -ForegroundColor DarkGray
Write-Host "TRIAD_TRANSFORM_NEGATIVE_INPUT_SHA_MISMATCH_V1_SELFTEST_OK" -ForegroundColor Green

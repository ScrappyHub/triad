...
$WorkRoot = Join-Path $RepoRoot "scripts\_work\stress_tail_partial_v1"
...
for($i=0; $i -lt $a.Length; $i++){ $a[$i] = [byte](($i * 11 + 7) % 251) }
$b = New-Object byte[] $chunk
for($i=0; $i -lt $b.Length; $i++){ $b[$i] = [byte](($i * 17 + 5) % 251) }
$tail = New-Object byte[] $tailSize
for($i=0; $i -lt $tail.Length; $i++){ $tail[$i] = [byte](($i * 19 + 3) % 251) }
...
Write-Utf8NoBomLf (Join-Path $InputRoot "notes.txt") "tail partial stress"
...
if($null -eq $PayloadEntry){ Die "TAIL_PARTIAL_PAYLOAD_ENTRY_NOT_FOUND" }
$Blocks = @(@($PayloadEntry.blocks))
if($Blocks.Count -lt 3){ Die ("TAIL_PARTIAL_BLOCKCOUNT_TOO_LOW: " + $Blocks.Count) }
$LastBlock = $Blocks[$Blocks.Count - 1]
$LastSize = [int64]$LastBlock.size
if($LastSize -ge $chunk){ Die ("TAIL_PARTIAL_NOT_OBSERVED: " + $LastSize) }
if($LastSize -ne $tailSize){ Die ("TAIL_PARTIAL_SIZE_MISMATCH: got=" + $LastSize + " expected=" + $tailSize) }
$ExpectedLen = [int64]$PayloadEntry.length
$ActualLen = (Get-Item -LiteralPath $RestoreOutFile).Length
if($ActualLen -ne $ExpectedLen){ Die ("RESTORED_LENGTH_MISMATCH: got=" + $ActualLen + " expected=" + $ExpectedLen) }
...
Write-Host ("TAIL_BLOCK_SIZE: " + $LastSize) -ForegroundColor DarkGray
Write-Host ("RESTORED_SHA: " + $RestoredSha) -ForegroundColor DarkGray
Write-Host "TRIAD_RESTORE_STRESS_TAIL_PARTIAL_V1_OK" -ForegroundColor Green

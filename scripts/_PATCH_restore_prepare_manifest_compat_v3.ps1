...
$BackupDir = Join-Path $ScriptsDir ("_backup_manifest_compat_v3_" + $ts)
...
foreach($p in $targets){
  ...
  if (-not $m.Success) { Die ("MANIFEST_COMPAT_V3_CANNOT_DETECT_LHS: " + $p + " line=" + ($idx+1)) }
  $lhs = $m.Groups[1].Value
  $indent = ([regex]::Match($lines[$idx], '^(\s*)')).Groups[1].Value

  # IMPORTANT: emit literal `$SnapshotDir etc (no interpolation in patcher)
  $block = New-Object System.Collections.Generic.List[string]
  [void]$block.Add($indent + "# Manifest resolution (compat v3; deterministic)")
